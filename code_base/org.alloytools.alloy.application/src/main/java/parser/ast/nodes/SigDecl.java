    ...
    public SigDecl(Node parent, Sig sig) {
        ...
        if (!isTopLevel) {
            ...
            if (isSubsig) { // S extends P
                this.parentName = StringUtil.afterSubstring(((Sig.PrimSig) sig).parent.label, Names.SLASH, true);
            } else { // S in P
                this.parentName = StringUtil.afterSubstring(((Sig.SubsetSig) sig).parents.get(0).label, Names.SLASH, true);
            }
        }
        this.fieldList = sig.getFieldDecls().makeConstList().stream().map(fieldDecl -> new FieldDecl(this, fieldDecl)).collect(Collectors.toList());
        ...
        // Add children.
        children.addAll(fieldList);
        if (hasSigFact) {
            children.add(sigFact);
        }
    }

    public SigDecl(Node parent) {
        super(parent);
    }
